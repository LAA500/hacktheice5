...
use App\Cart\Core\CoreCartItem;
use App\Exceptions\ItemMissing;
...
trait CartItemsManager
{
    ...
    public function add($entity, $quantity)
    {
        ...
        $this->items->push(CoreCartItem::createFrom($entity, $quantity));
     ...
     * 
     */
    public function setPeriodsAt($cartItemIndex, $periods)
    {
        $this->existenceCheckFor($cartItemIndex);

        $this->items[$cartItemIndex]->periods = $periods;

        $this->cartDriver->setCartItemPeriods(
            $this->items[$cartItemIndex]->id,
            $this->items[$cartItemIndex]->periods
        );

        cart()->refreshAllPrice();
    ...
    public function refreshAllItemsData()
    {
        ...
        $this->items->transform(function ($item) use (&$keepDiscount) {
            ...
            $cartItem = CoreCartItem::createFrom($freshEntity, $item->quantity);
    ...
    /**
     * Refreshes all prices data.
     *
     * @return array
     */
    public function refreshAllPrice()
    {
        $keepDiscount = true;
        ...
        $this->items->transform(function ($item) use (&$keepDiscount) {
            ...
            if ($freshEntity->is_rent) {
                ...
                $item->price = discountCalculation($freshEntity, $item->periods) * $item->periods;
            }
